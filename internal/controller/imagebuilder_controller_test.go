...
var _ = Describe("ImageBuilder Controller", func() {
	Context("When reconciling a resource", func() {
		...
		imagebuilder := &pokv1alpha1.ImageBuilder{}

		BeforeEach(func() {
			By("creating the custom resource for the Kind ImageBuilder")
			err := k8sClient.Get(ctx, typeNamespacedName, imagebuilder)
			if err != nil && errors.IsNotFound(err) {
				resource := &pokv1alpha1.ImageBuilder{
		...
		AfterEach(func() {
			...
			By("Cleanup the specific resource instance ImageBuilder")
